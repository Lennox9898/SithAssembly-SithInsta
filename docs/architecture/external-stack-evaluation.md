...
Date: 2026-09-04. Printable snapshot: `docs/External-Stack-Evaluation-Dossier-2026-09-04.pdf`.
...
| Commands | `src/SithAssembly/CommandDeck/command_engine.py`, `SithAssembly.Runtime.py` | Allowlisted local commands against the case store, not a general shell or tool proxy. |
| Registry and modules | `config/module_registry.json`, `src/module_runtime.py`, `src/assembly_manifest.py` | Explicit `src.SithAssembly.*` declarations, controlled loading, and a readable manifest. |
| Coordination | `config/agent_registry.json`, `src/SithAssembly/Conclave/`, `src/SithAssembly/AssemblyCore/agent_controller.py` | Local topic and capability model; no external message broker. |
| Data and graph | `src/database.py`, `src/repository.py`, `src/SithAssembly/SpectreNet/`, `src/SithAssembly/GhostCluster/` | SQLite, evidence-bound edges, local groups, and centrality. |
| Analysis | `src/SithAssembly/SignalForge/`, `src/SithAssembly/GlyphWatch/`, `src/SithAssembly/MindForge/` | Optional local adapters; no silent model downloads or external calls. |
| Evidence and exports | `src/SithAssembly/CipherLedger/`, `src/SithAssembly/EvidenceVault/`, `src/SithAssembly/BlackArchive/` | Fingerprints, opt-in encrypted packages, and JSON/PDF exports. |
| Telemetry | `src/runtime_logging.py`, `src/SithAssembly/ForgeProbe/runtime_doctor.py` | Local redacted JSONL logs and read-only runtime diagnostics. |
| Deployment | `config/deployment.local.json`, `src/SithAssembly/Citadel/deployment_preflight.py`, `deploy/` | Inactive preparation topology; no cluster is running. |
...
| OpenLineage and Rekor | Reuse concepts | `src/runtime_logging.py`, `src/SithAssembly/CipherLedger/`, and `src/SithAssembly/EvidenceVault/` are the basis. Specify an internal run/artifact envelope first. |
...
| QUT Coordination Network Toolkit | Evaluate methodology | Test platform-neutral coordination features against `src/pattern_engine.py` and `src/SithAssembly/SignalForge/`. Results remain review-required pattern candidates. |
